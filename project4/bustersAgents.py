...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for belief in livingGhostPositionDistributions:
            distributions.append(belief.argMax())
        goal = None
        prob = 0
        for num, state in enumerate(distributions):
            # print(num)
            # print(state)
            if livingGhostPositionDistributions[num][state] >= prob:
                goal = state
                prob = livingGhostPositionDistributions[num][state]
        nextactions = []
        for action in legal:
            nextposition = Actions.getSuccessor(pacmanPosition, action)
            nextactions.append((self.distancer.getDistance(nextposition, goal), action))
        nextaction = min(nextactions)
        return nextaction[1]
