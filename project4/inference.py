...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        noisyalldis = gameState.getNoisyGhostDistances()
        if noisyDistance is None:
            # allPossible is {} first
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        for i in range(self.numParticles):
            ranpos = ([random.choice(self.legalPositions) for j in range(self.numGhosts)])
            self.particles.append(tuple(ranpos))
    ...
    def elapseTime(self, gameState):
        ...
        for currPart in self.particles:
            ...
            for agent in range(self.numGhosts):
                newDist = getPositionDistributionForGhost(setGhostPositions(gameState, prevGhostPositions), agent,
                                                          self.ghostAgents[agent])
                newPart[agent] = util.sample(newDist)
            particles.append(tuple(newPart))
